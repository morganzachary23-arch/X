    ...
    while True:
        try:
            headers = {
                ...
                'Referer': 'https://jeetbuzz.org/bd/en/register', 
                'sec-ch-ua-full-version-list': '\"Chromium\";v=\"139.0.7339.0\", \"Not;A=Brand\";v=\"99.0.0.0\"', 
                'sec-ch-ua-bitness': '\"\"', 
                'sec-ch-ua-model': '\"LE2101\"', 
                'sec-ch-ua-platform': '\"Android\"'
            }
            url = 'https://jeetbuzz.org/api/wv/v1/user/registerPreCheck'
            json_data = {
                'languageTypeId': 1, 
                'currencyTypeId': 8, 
                'userId': uname, 
            ...
            conn = http.client.HTTPSConnection('jeetbuzz.org')
            ...
            if data['status'] == 'F0003':
                ...
                time.sleep(30)
                continue  # Retry the same user like dex.py
            ...
            time.sleep(10)
            print(f'{R} ERROR >> {e}{D}')
            continue  # Retry on error like dex.py

def check_username(username):
    uname = username.replace(' ', '').split('|')[0]
    if checker(uname):
        print(f'{BOLD}{G} [IG] {uname}{D}')
...
def main():
    ...
    print(f'\n{BOLD}{G} TOTAL{Y}{total}{G} VALID IG IDS FOUND{D}\n\n')
